...
const CONFIG = {
  dataPath: 'packs',
  sourcePath: '_source',
  databases: ['armors', 'careers', 'gears', 'obligations', 'pregens', 'species', 'specializations', 'talents', 'weapons'],
  yaml: true,
}
...
startup.command('compile').description('Constructs binary databases from plain-text source files.').action(compile)

/* Plain-text format extraction from LevelDB directory */
startup.command('extract').description('Unpacks binary databases into plain-text source files.').action(extract)
