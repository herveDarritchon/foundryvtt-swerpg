 ...
 * @property {SwerpgItem} [weapon]        A specific weapon item being used
 */
...
/**
 * @typedef {Object} SwerpgActionOutcome
 * @property {SwerpgActor} target       The outcome target
...
/**
 * @typedef {Map<SwerpgActor,SwerpgActionOutcome>} SwerpgActionOutcomes
 */
 ...
 */
export default class SwerpgAction extends foundry.abstract.DataModel {
  static defineSchema() {
    ...
    return {
      ...
      target: new fields.SchemaField({
        ...
        scope: new fields.NumberField({
          required: true,
          initial: SYSTEM.ACTION.TARGET_SCOPES.ALL,
      ...
      actionHooks: new fields.ArrayField(
   ...
   * @type {undefined|SwerpgActionOutcomes}
   */
  outcomes
  ...
  /**
   * One-time configuration of the SwerpgAction as part of construction.
   * @param {object} [options]            Options passed to the constructor context
   * @param {SwerpgActor} [options.actor]   A specific actor to whom this action is bound
   * @param {SwerpgTokenObject} [options.token]  A specific token performing this Action
   ...
   * @param {SwerpgActor} actor     The Actor to which the action should be attached
   * @returns {SwerpgAction}        This action, for chaining
   */
  bind(actor) {
   ...
   * @param {SwerpgTokenObject} [options.token]     A specific Token which is performing the action
   ...
   * @returns {Promise<SwerpgActionOutcomes|undefined>}
   */
  async use({ token, ...options } = {}) {
    if (!this.actor) throw new Error('A SwerpgAction may not be used unless it is bound to an Actor')

    // Redirect to spellcasting
    if (this.id === 'cast') {
      const spell = swerpg.api.models.SwerpgSpellAction.getDefault(this.actor)
  ...
  async #use({ chatMessage = true, chatMessageOptions = {}, dialog = true } = {}) {
    ...
    await this.actor.update({ 'flags.swerpg': this.usage.actorFlags })
    ...
    // Other Target Types
      ...
      switch (this.target.type) {
        ...
        case 'summon':
          return []
        case 'self':
          targets = this.actor.getActiveTokens(true).map(SwerpgAction.#getTargetFromToken)
          break
        case 'single':
        ...
        default:
          ...
          targets = Array.from(game.user.targets).map(SwerpgAction.#getTargetFromToken)
          break
      }
    }
  ...
  #acquireTargetsFromTemplate() {
    ...
    const tokens = canvas.tokens.quadtree.getObjects(bounds, {
    ...
    const targets = Array.from(tokens).map(SwerpgAction.#getTargetFromToken)
  ...
  #acquireSingleTargets(strict) {
    ...
    if (tokens.size < 1) {
      if (strict)
        throw new Error(
          game.i18n.format('ACTION.WarningInvalidTarget', {
    ...
    for (const token of tokens) {
      const t = SwerpgAction.#getTargetFromToken(token)
      ...
      const range = swerpg.api.grid.getLinearRangeCost(this.token, token)
   ...
   */
  canAutoConfirm() {
    const autoConfirm = game.settings.get('swerpg', 'autoConfirm')
   ...
   * @param {SwerpgActionOutcomes} outcome    The outcome being prepared
   ...
   * @returns {SwerpgActionOutcome}           The outcome for the Actor
   */
  #createSelfOutcome() {
   ...
   * @param {SwerpgActionOutcome} outcome     The outcome being finalized
   */
  ...
  *_tests() {
  ...
  _canUse(targets = []) {
    ...
    if (this.cost.action && this.actor.isIncapacitated) {
      throw new Error(
        game.i18n.format('ACTION.WarningCannotSpendAction', {
    ...
    if (this.cost.action > r.action.value) {
      throw new Error(
        game.i18n.format('ACTION.WarningCannotAffordCost', {
          name: this.actor.name,
    ...
    if (this.cost.focus) {
      ...
      if (focusBlock)
        throw new Error(
          game.i18n.format('ACTION.WarningCannotSpendFocus', {
            name: this.actor.name,
    ...
    for (const test of this._tests()) {
      ...
      if (can === false)
        throw new Error(
          game.i18n.format('ACTION.WarningCannotUseTag', {
            name: this.actor.name,
            action: this.name,
            tag: test.label || '',
          }),
   ...
   * @param {SwerpgActionOutcome} outcome
  ...
  getTags() {
    ...
    if (cost.weapon && !this.actor) {
      // Un-owned actions which include weapon cost
  ...
  static async confirm(message, { action, reverse = false } = {}) {
    ...
    return message.update({ flags: { swerpg: flagsUpdate } })
  }
   ...
   * @returns {SwerpgAction}        The reconstituted Action instance
   */
  static fromChatMessage(message) {
    const { action: actionId, template: templateId, outcomes } = message.flags.swerpg || {}
    if (!actionId) throw new Error(`ChatMessage ${message.id} does not contain SwerpgAction data`)

    // Get the Actor and Action
    const actor = ChatMessage.getSpeakerActor(message.speaker)
    let action
    if (actionId.startsWith('spell.')) {
      action = game.system.api.models.SwerpgSpellAction.fromId(actionId, { actor })
  ...
  static async onDeleteChatMessage(message, options, userId) {
    const template = message.flags.swerpg?.template
    if (!template) return
  ...
  getAnimationSequence() {
    ...
    const originToken = SwerpgAction.#getTargetToken(this.actor)
    ...
    if (this.target.type === 'single') {
      for (const outcome of this.outcomes.values()) {
        ...
        const targetToken = SwerpgAction.#getTargetToken(outcome.target)
        ...
        } else sequence.effect().file(config.src).atLocation(originToken).stretchTo(targetToken).missed(!hit).waitUntilFinished(wait)
  ...
  /**
   * Get the placeable Token location for a SwerpgActor in the current Scene, if any.
   * @param {SwerpgActor} actor       The Actor being targeted
   * @returns {Token|null}
   */
