 ...
 */
export default class SwerpgSkill extends foundry.abstract.TypeDataModel {
  /** @override */
  static defineSchema() {
    ...
    return {
      ...
      ranks: new fields.SchemaField(
        Object.values(SKILL.RANKS).reduce((ranks, rank) => {
      ...
      paths: new fields.SchemaField(
        Array.fromRange(3, 1).reduce((paths, i) => {
          paths[`path${i}`] = new fields.SchemaField({
            id: new fields.StringField(),
            name: new fields.StringField(),
            overview: new fields.HTMLField(),
            ranks: new fields.SchemaField(
              ['specialist', 'master'].reduce((ranks, rank) => {
  ...
  static async initialize() {
    ...
    for (const skill of Object.values(SKILL.SKILLS)) {
      foundry.utils.mergeObject(
        skill,
        {
          icon: `systems/swerpg/icons/skills/${skill.id}.jpg`,
      ...
      if (foundry.utils.isEmpty(skill.paths))
        skill.paths = Array.fromRange(3, 1).reduce((paths, i) => {
          paths[`path${i}`] = {
            id: `path${i}`,
