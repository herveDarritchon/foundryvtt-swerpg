 ...
 *
 * @property {SwerpgRune} rune
 * @property {SwerpgGesture} gesture
 * @property {SwerpgInflection} inflection
 * @property {number} composition
 * @property {string} damageType
 */
export default class SwerpgSpellAction extends SwerpgAction {
  static defineSchema() {
  ...
  *_tests() {
  ...
  _prepareData() {
    ...
    // Composed Spell
    if (this.composition >= SwerpgSpellAction.COMPOSITION_STATES.COMPOSING) {
      ...
      this.name = SwerpgSpellAction.#getName(this)
    ...
    this.cost = SwerpgSpellAction.#prepareCost.call(this)
    this.defense = SwerpgSpellAction.#prepareDefense.call(this)
    this.damage = SwerpgSpellAction.#prepareDamage.call(this)
   ...
   * @this {SwerpgSpellAction}  The spell being prepared
  ...
  /** @inheritDoc */
  _prepare() {
    SwerpgSpellAction.#prepareGesture.call(this)
    super._prepare()
    ...
    if (this.composition !== SwerpgSpellAction.COMPOSITION_STATES.COMPOSED) {
  ...
  acquireTargets(options = {}) {
    if (this.composition === SwerpgSpellAction.COMPOSITION_STATES.COMPOSING) options.strict = false
  ...
  clone(updateData = {}, context) {
    if (!this.composition) updateData.composition = SwerpgSpellAction.COMPOSITION_STATES.COMPOSING
   ...
   * @param {SwerpgActor} actor       The Actor for whom the spell is being prepared
   * @param {object} spellData          Initial data for the spell
   * @returns {SwerpgSpellAction}     The constructed SwerpgSpellAction
   */
  static getDefault(actor, spellData = {}) {
    // Repeat Last Spell
    const lastSpell = actor.flags.swerpg?.lastSpell
    if (lastSpell) {
      try {
   ...
   * @returns {SwerpgSpellAction}   The constructed spell instance
   */
  ...
  async configure(targets) {
    ...
    this.updateSource({ composition: SwerpgSpellAction.COMPOSITION_STATES.COMPOSED })
    return result
  }
  ...
  getTags() {
    ...
    // Variable Cost
    if (this.composition === SwerpgSpellAction.COMPOSITION_STATES.NONE) {
    ...
    tags.action.scaling = Array.from(this.scaling)
      .map((a) => SYSTEM.CHARACTERISTICS[a].label)
      .join('/')
  ...
  _getAnimationConfiguration() {
    return SwerpgSpellAction.ANIMATION_CONFIG[this.gesture.id]?.[this.rune.id]
  }
