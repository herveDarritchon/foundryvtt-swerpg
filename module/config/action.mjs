...
export const TAGS = {
  ...
  reaction: {
    ...
    prepare() {
      const a = this.actor
      const canFreeReact = a.talentIds.has('gladiator0000000') && !a.system.status.gladiator && (this.tags.has('mainhand') || this.tags.has('offhand'))
      if (canFreeReact) {
  ...
  summon: {
    ...
    async confirm() {
      ...
      // Create a Token
      let token = await summonActor.getTokenDocument({
        x,
        y,
      ...
      if (ae) foundry.utils.setProperty(ae, 'flags.swerpg.summon', token.uuid)
...
export const DEFAULT_ACTIONS = Object.freeze([
  ...
  // Defend
  {
    ...
    description:
      'You concentrate effort on avoiding harm, heightening your physical defense. You gain the ' +
  ...
  // Delay
  {
    ...
    description:
      'You delay your action until a later point in the Combat round. Choose an Initiative between 1 and ' +
    ...
    _hooks: {
      canUse(_targets) {
        ...
        if (this.actor.flags.swerpg?.delay) {
          throw new Error('You may not delay your turn again this combat round.')
        }
      },
      displayOnSheet(combatant) {
        if (!combatant || this.actor.flags.swerpg?.delay || game.combat.combatant !== combatant) return false
