 ...
 */
export default class SwerpgTalentChoiceWheel extends PIXI.Container {
  constructor() {
    super()
   ...
   * @type {SwerpgTalentTreeNode|null}
   */
  node = null
   ...
   * @param {SwerpgTalentTreeNode} node
   ...
   */
  #drawBackground() {
    this.bg.texture = getTexture('systems/swerpg/ui/tree/wheel.webp')
   ...
   */
  #drawEdges() {
    this.edges.clear().lineStyle({ color: this.node.config.borderColor, width: 3 }).drawCircle(0, 0, this.radius).endFill()
  ...
  async #drawTalents() {
    ...
    for (const talent of talents) {
      ...
      const icon = new SwerpgTalentTreeTalent(this.node, talent, position, {
