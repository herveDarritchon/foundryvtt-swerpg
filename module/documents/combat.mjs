export default class SwerpgCombat extends Combat {
  /**
   ...
   * @type {number}
   */
  #priorHeroism = game.settings.get('swerpg', 'heroism') || 0
  ...
  async #awardHeroism() {
    ...
    if (this.round === 1) {
      this.#priorHeroism = 0
      return game.settings.set('swerpg', 'heroism', 0)
    }
    ...
    const heroism = game.settings.get('swerpg', 'heroism') || 0
  ...
  #postInitiativeMessage(round, results) {
    ...
    const rows = results
      .map((i) => {
        ...
        const modifiers = [rd.ability.signedString(), rd.boons > 0 ? `${rd.boons} Boons` : '', rd.banes > 0 ? `${rd.banes} Banes` : ''].filterJoin(', ')
    ...
    return ChatMessage.create({
      content: `
      <section class="swerpg dice-roll initiative">
      ...
      'flags.swerpg.isInitiativeReport': true,
    })
  }
}
