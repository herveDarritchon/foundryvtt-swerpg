...
export default class SwerpgActor extends Actor {
     ...
     * @type {Object<string, {talent: SwerpgTalent, fn: Function}[]>}
     */
    actorHooks = {};
    ...
    prepareEmbeddedDocuments() {
        ...
        SwerpgActor.#prepareActions.call(this);
    };
     ...
     * @this {CrucbileActor}
     * @returns {SwerpgActorTraining}   Prepared training ranks in various equipment categories
     */
...
//            natural: Math.clamp(Math.floor((this.system.advancement.level + 1) / 4), 0, 3) // TODO temporary
     ...
     * @this {SwerpgActor}
     * @param {object} items
     * @param {SwerpgItem[]} items.armor
     * @param {SwerpgItem[]} items.weapon
     * @param {SwerpgItem[]} items.accessory
    ...
    static #prepareEquipment({armor, weapon, accessory} = {}) {
        ...
        equipment.canFreeMove = SwerpgActor.#canFreeMove(this, equipment.armor);
    ...
    static #prepareActions() {
        this.actions = {};
        SwerpgActor.#prepareDefaultActions.call(this);
    ...
    static #prepareDefaultActions() {
        ...
        for (let ad of SYSTEM.ACTION.DEFAULT_ACTIONS) {
            ...
            // Create the action
            const action = new SwerpgAction(ad, {actor: this});
            action._initialize({});
     ...
     * @param {SwerpgActor} actor   The Actor being prepared
    ...
    /**
     * A wrapper around SwerpgActor#useAction which encapsulates some Macro hotbar behaviors.
     * @param {SwerpgActor} actor   The Actor which should take action
     ...
     * @param {SwerpgAction} action       The Skill Attack action being performed
     ...
     * @param {SwerpgAction} action         The action being performed
     * @param {SwerpgActor} target          The target being attacked
     * @param {SwerpgItem} [weapon]         The weapon used in the attack
    ...
    async delay(initiative, actorUpdates = {}) {
        ...
        await this.update(foundry.utils.mergeObject(actorUpdates, {
            "flags.swerpg.delay": {
     ...
     * @returns {Promise<SwerpgActor>}            The updated Actor document
     */
     ...
     * @param {SwerpgAction} action             The Action being applied
     * @param {SwerpgActionOutcome} outcome     The Action outcome
     ...
     * @param {SwerpgAction} action                The action performed
     * @param {SwerpgActionOutcomes} outcomes      The action outcomes that occurred
     */
    ...
    async resetTalents({dialog = true} = {}) {
        ...
        if (dialog) {
            // Render confirmation content from an external Handlebars template so HTML is separated from JS
            const content = awaitfoundry.applications.handlebars.renderTemplate(`systems/${SYSTEM.id}/templates/dialogs/confirm-reset-talents.hbs`, {
                actorName: this.name
            });
    ...
    async purchaseCharacteristic(characteristicId, action) {
        ...
        const characteristicClass = CharacteristicFactory.build(this, characteristicId, {
            action,
            isCreation: true,
        }, {});
