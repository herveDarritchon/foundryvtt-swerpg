...
export function addChatMessageContextOptions(html, options) {
    ...
    options.push({
        ...
        callback: async li => {
            const message = game.messages.get(li.data("messageId"));
            return SwerpgAction.confirm(message);
        }
    });
    ...
    options.push({
        ...
        callback: async li => {
            const message = game.messages.get(li.data("messageId"));
            return SwerpgAction.confirm(message, {reverse: true});
        }
    });
    return options;
}
...
export async function onKeyboardConfirmAction(context) {
    ...
    for (const id of messageIds) {
        ...
        if (seconds > 60) break;
        const {action, confirmed} = message.flags.swerpg || {};
        if (action && !confirmed) toConfirm.unshift(message);
    }
    if (toConfirm.length) return SwerpgAction.confirm(toConfirm[0]);
}
