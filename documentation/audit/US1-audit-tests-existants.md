...
**226 tests uniques** couvrent les couches métier (SkillFactory, calculs de coûts, achat/retrait de rangs). **Zéro test
** ne couvre la couche de présentation (préparation des données sheet, rendu template).
...
- `module/utils/__tests__/skill-costs.test.js` (20 tests) — doublon de `tests/utils/skill-costs.test.mjs`, probablement
  une relique d'une ancienne structure.
...
1. **Tester `#prepareSkills()`** — c'est le point d'entrée principal. Vérifier que les données produites sont
   cohérentes (catégories `exp`/`kno`/`soc`, champs `nextRank`/`nextCost`/`canPurchase` présents, pips corrects).

2. **Tester `_prepareContext()` partie skills** — s'assurer que les compteurs `freeSkillRanks.*.available` sont
   corrects.
