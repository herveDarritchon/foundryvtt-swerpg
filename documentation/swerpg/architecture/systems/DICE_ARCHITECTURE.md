...
export class SwerpgDiceRoll extends Roll {
  ...
  _calculateNetResults() {
    this.narrativeResult.netSuccess = this.narrativeResult.success - this.narrativeResult.failure
    this.narrativeResult.netAdvantage = this.narrativeResult.advantage - this.narrativeResult.threat
  }
...
export class SwerpgDiceDialog extends Dialog {
  ...
  async _renderInner() {
    ...
    const content = await renderTemplate('systems/swerpg/templates/dice/dice-dialog.hbs', context)
...
export class SwerpgAction {
  ...
  _applyTalentModifiers(pool) {
    ...
    const relevantTalents = actor.items.filter((i) => i.type === 'talent' && i.system.effects.some((e) => e.target === this.skill))
  ...
  _applyTalentEffect(pool, effect, talent) {
    switch (effect.type) {
      ...
      case 'reduce_difficulty':
        pool.dice.difficulty = Math.max(0, pool.dice.difficulty - parseInt(effect.value) * talent.system.currentRank)
        break
    }
  }
