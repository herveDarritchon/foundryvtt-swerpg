...
### Personnalisation “swerpg-like” (onglets sur le côté)

Cette section détaille les réglages et variantes utiles quand tu veux reproduire le rendu swerpg
(barre verticale collée au bord et partiellement “hors cadre”). L’objectif est d’avoir un style
cohérent, robuste (pas coupé par l’overflow) et utilisable au clavier.

#### Markup recommandé : `<button>` plutôt que `<a>` (optionnel mais conseillé)

Dans beaucoup de templates Foundry, les onglets sont rendus avec des `<a>`. C’est compatible,
mais un onglet “déclenche une action” et ne navigue pas vers une URL : un `<button type="button">`
reflète mieux la sémantique (et évite les comportements de lien).

Exemple (mêmes attributs ApplicationV2, version bouton) :
...
- Le nom accessible doit contenir le libellé visible (ici : le tooltip) pour la commande vocale.
- L’image est décorative : `alt=""` + `aria-hidden="true"`.
- Évite `aria-role` : ce n’est pas un attribut ARIA valide.

#### Variables CSS : rendre le design facilement “thémable”

Plutôt que de figer des pixels partout, expose des variables afin d’ajuster rapidement la taille,
la position, et les couleurs :

```less
/* Scope recommandé : la sheet concernée (ex. actor sheet) */
.swerpg.sheet.actor {
  --sheet-tab-size: 32px;
  --sheet-tab-hit-extra: 4px;
  --sheet-tab-icon-size: 28px;
  --sheet-tab-top: 90px;
  --sheet-tab-gap: var(--margin-half);
  --sheet-tab-bg: var(--color-frame-bg-75);
  --sheet-tab-bg-active: var(--color-frame-bg);
  --sheet-tab-border: #000;
  --sheet-tab-border-active: var(--color-accent);
  --sheet-tab-focus: var(--color-warm-1);
}
```

Puis consomme-les dans la nav :

```less
.sheet-tabs {
  --tab-size: var(--sheet-tab-size);
  --hit-extra: var(--sheet-tab-hit-extra);
  --icon-size: var(--sheet-tab-icon-size);

  top: var(--sheet-tab-top);
  gap: var(--sheet-tab-gap);
  right: calc(-1 * (var(--tab-size) + var(--hit-extra)));
  width: calc(var(--tab-size) + var(--hit-extra));

  > a,
  > button {
    width: var(--tab-size);
    height: calc(var(--tab-size) + var(--hit-extra));
    background: var(--sheet-tab-bg);
    border-color: var(--sheet-tab-border);

    &.active {
      background: var(--sheet-tab-bg-active);
      border-color: var(--sheet-tab-border-active);
    }

    &:focus-visible {
      outline-color: var(--sheet-tab-focus);
    }
  }
}
```

#### Positionnement : z-index, overflow et “hors cadre”

Le rendu “hors cadre” dépend de l’overflow des conteneurs. Si la nav est coupée :

- Vérifie si un parent impose `overflow: hidden`.
- Si tu contrôles le conteneur proche, préfère `overflow: visible` sur le wrapper de la sheet.
- Donne un `z-index` à la nav si elle passe derrière d’autres éléments.

Exemple (à adapter au DOM réel de ta sheet) :
...
#### Responsive : fallback horizontal (recommandé)

Sur des fenêtres étroites, le “right négatif” peut réduire la découvrabilité et la cible cliquable.
Un fallback simple consiste à basculer la nav en barre horizontale à l’intérieur du cadre.
...
Si tu utilises `position: absolute` sur la nav, tu peux aussi faire un repositionnement “en bas”
(`bottom: 0; top: auto;`) selon ton layout.

#### Animations : respecter `prefers-reduced-motion`

Le style swerpg est souvent plus agréable avec une transition légère (hover/active), mais il faut
désactiver les animations si l’utilisateur a demandé une réduction des mouvements.
...
#### Variante “icône + texte” (accessibilité / apprentissage)

Si le système est destiné à des personnes découvrant l’UI, une variante “icône + libellé” peut
réduire la charge cognitive.

- Garde l’icône.
- Ajoute un `<span class="tab-label">{{localize tab.label}}</span>`.
- En CSS, tu peux masquer le texte sur grandes fenêtres et l’afficher uniquement en fallback.

L’important : ne jamais dépendre uniquement de la couleur/opacity pour distinguer l’état actif.
