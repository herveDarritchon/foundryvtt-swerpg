...
export class SwerpgBaseActorSheet extends api.HandlebarsApplicationMixin(sheets.ActorSheetV2) {
  ...
  async _prepareContext(options) {
    ...
    context.enrichedDescription = await TextEditor.enrichHTML(this.document.system.details?.description || '', { async: true })
...
export class SwerpgSocket {
  ...
  static async _handleDestinyPoolUpdate(payload) {
    ...
    ui.notifications.info(game.i18n.localize('SWERPG.DestinyPool.Updated'))
...
export class SwerpgActor extends Actor {
  constructor(...args) {
    ...
    this._debouncedRecalculate = foundry.utils.debounce(this._recalculateDerived.bind(this), 100)
  }
  ...
  async _recalculateDerived() {
    ...
    updateData['system.derived.wounds.max'] = this.system.characteristics.brawn + 10
    updateData['system.derived.strain.max'] = this.system.characteristics.willpower + 10
