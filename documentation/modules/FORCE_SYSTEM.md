  ...
  static async rollForcePool(forceRating, difficulty = 0) {
    ...
    for (const die of forceDice) {
      for (const result of die.results) {
        ...
        if (face <= 2)
          lightPoints += 1 // 1-2: 1 point lumineux
        else if (face <= 4)
          lightPoints += 2 // 3-4: 2 points lumineux
        else if (face <= 6)
          darkPoints += 1 // 5-6: 1 point obscur
...
class ForcePowerActivation {
  ...
  async activate(targets = []) {
    ...
    const forceResult = await ForcePointGeneration.spendForcePoints(this.actor, this.totalCost, this.actor.system.force.alignment)
...
class InfluencePowerEffect {
  ...
  static async influenceEmotion(actor, target, upgrades) {
    ...
    const actorPool = DicePoolBuilder.buildPool(actor.system.characteristics.willpower, actor.system.skills.deception?.rank || 0, 0)

    const targetPool = DicePoolBuilder.buildPool(0, 0, target.actor.system.characteristics.willpower)
...
class MoralitySystem {
  ...
  static async triggerDarkSideTransition(actor) {
    ...
    ui.notifications.warn(
      game.i18n.format('SWERPG.Force.DarkSideTransition', {
        actor: actor.name,
      }),
    )
...
class ConflictGeneration {
  ...
  static async generateConflict(actor, source, customAmount = null) {
    ...
    // Notification
    ui.notifications.warn(
      game.i18n.format('SWERPG.Force.ConflictGenerated', {
        amount: amount,
...
class ForceCombat {
  ...
  static async resolveLightsaberDeflection(actor, attackRoll) {
    ...
    const sensePower = actor.items.find((i) => i.type === 'forcepower' && i.system.identifier === 'sense')
    ...
    const deflectionPool = DicePoolBuilder.buildPool(actor.system.characteristics.brawn, actor.system.skills.lightsaber?.rank || 0, attackRoll.difficulty)
