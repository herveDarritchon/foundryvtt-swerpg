...
class ObligationEffects {
  ...
  static async applyMinorEffects(actor, obligation) {
    ...
    await actor.update({
      'system.stress.strain.value': Math.min(actor.system.stress.strain.max, actor.system.stress.strain.value + 2),
    })
  ...
  static async applyMajorEffects(actor, obligation) {
    ...
    await actor.update({
      'system.stress.strain.value': Math.min(actor.system.stress.strain.max, actor.system.stress.strain.value + 4),
    })
...
class DutyProgression {
  static async awardDuty(actor, amount, reason) {
    ...
    ui.notifications.info(
      game.i18n.format('SWERPG.Duty.Awarded', {
        amount: amount,
        total: newDuty,
        actor: actor.name,
      }),
...
class ConflictGeneration {
  static async generateConflict(actor, source, amount = 1) {
    ...
    ui.notifications.warn(
      game.i18n.format('SWERPG.Conflict.Generated', {
        amount: amount,
        source: source,
        total: newConflict,
      }),
...
class ObligationDialog extends Dialog {
  ...
  async _onCreate(html) {
    ...
    ui.notifications.info(
      game.i18n.format('SWERPG.Obligation.Created', {
        name: obligationData.name,
      }),
    )
  }
...
class SessionObligationTracker {
  ...
  static async recordObligationEvent(obligationResult) {
    const journalEntry = game.journal.getName('Session Log') || (await JournalEntry.create({ name: 'Session Log' }))
