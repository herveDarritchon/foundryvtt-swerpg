...
**Module(s) impacté(s)** : `module/utils/talent-tree-resolver.mjs` (création),
`module/models/character.mjs` (modification),
`module/applications/sheets/character-sheet.mjs` (modification),
`tests/utils/talent-tree-resolver.test.mjs` (création),
...
- Création de `module/utils/talent-tree-resolver.mjs` avec :
...
**Quoi** : créer `module/utils/talent-tree-resolver.mjs` pour centraliser la logique de résolution spécialisation → arbre.
...
**Fichiers** :
- `module/utils/talent-tree-resolver.mjs` (création)
...
**Fichiers** :
- `module/utils/talent-tree-resolver.mjs` (création — intégré dans la logique)
- Tests associés
...
Tests du resolver (`tests/utils/talent-tree-resolver.test.mjs`) :
...
| `module/utils/talent-tree-resolver.mjs` | Création | Module de résolution spécialisation → arbre : `resolveSpecializationTree()`, `resolveActorSpecializationTrees()`, `getSpecializationTreeResolutionState()`. Résolution primaire par `treeUuid`, fallback par `specializationId`. Gestion des cas dégradés (`unresolved`, `incomplete`). |
...
| `tests/utils/talent-tree-resolver.test.mjs` | Création | Tests unitaires du resolver : résolution par UUID, fallback, cas dégradés, arbre incomplet. |
